...
async def get_or_create_room(
    ...
    room = await chat_service.get_existing_room_or_create(db, current_user_id, opponent_id)
    return room
...
async def list_my_rooms(
    ...
    rooms = await chat_service.get_user_chat_rooms(db, current_user_id)
    return rooms
...
async def send_message(
    ...
    new_msg = await chat_service.save_new_message(db, room_id, current_user_id, msg_in.content)
    return new_msg
...
async def get_messages(
    ...
    messages = await chat_service.get_messages_after_id(db, room_id, last_id, limit)
    return messages
...
async def mark_messages_as_read(
    ...
    await chat_service.update_messages_read_status(db, room_id, current_user_id)
...
async def get_opponent_status(
    ...
    status_info = await chat_service.get_chat_partner_status(db, room_id, current_user_id)
    return status_info