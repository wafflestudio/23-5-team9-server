...
class PayRepository:
    ...
    async def get_ledgers(self, user_id: str, limit: int, offset: int) -> list[Ledger]:
        stmt = (
            select(Ledger)
            .where(or_(Ledger.user_id == user_id, Ledger.receive_user_id == user_id))
            .order_by(desc(Ledger.time))
            .limit(limit)
            .offset(offset)
            .options(selectinload(Ledger.receive_user))
        )
